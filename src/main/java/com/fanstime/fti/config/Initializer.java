...
/**
 * Created by Jay Nicolas on 13.05.2018.
 */
class Initializer implements BeanPostProcessor {
    ...
    private void initConfig(SystemProperties config) {
        ...
        if (logger.isInfoEnabled()) {
            logger.info("capability fti version: [{}]", "62, 63");
        }
        logger.info("capability shh version: [{}]", 3);
        logger.info("capability bzz version: [{}]", 0);
    ...
    public static class DatabaseVersionHandler {
        ...
        public boolean isDatabaseDirectoryExists(SystemProperties config) {
            ...
            for (String str : databaseFile.list()) {
                if (str.equals("version.properties")) {
                    return true;
                }
            }
            return false;
