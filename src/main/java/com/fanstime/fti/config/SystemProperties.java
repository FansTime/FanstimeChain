...
import com.fanstime.fti.config.blockchain.OlympicConfig;
import com.fanstime.fti.config.net.*;
import com.fanstime.fti.core.Genesis;
import com.fanstime.fti.core.genesis.GenesisConfig;
import com.fanstime.fti.core.genesis.GenesisJson;
...
/**
 * Utility class to retrieve property values from the fti.conf files
 * <p>
 ...
 * - resource fti.conf : normally used as a reference config with default values
 * and shouldn't be changed
 * - system property : each config entry might be altered via -D VM option
 * - [user dir]/config/fti.conf
 * - config specified with the -Dfti.conf.file=[file.conf] VM option
 * - CLI options
 */
public class SystemProperties {
    ...
    public SystemProperties(Config apiConfig, ClassLoader classLoader) {
        try {
            ...
            Config referenceConfig = ConfigFactory.parseResources("fti.conf");
            logger.info("Config (" + (referenceConfig.entrySet().size() > 0 ? " yes " : " no  ") + "): default properties from resource 'fti.conf'");
            String res = System.getProperty("fti.conf.res");
            Config cmdLineConfigRes = res != null ? ConfigFactory.parseResources(res) : ConfigFactory.empty();
            logger.info("Config (" + (cmdLineConfigRes.entrySet().size() > 0 ? " yes " : " no  ") + "): user properties from -Dfti.conf.res resource '" + res + "'");
            ...
            File userDirFile = new File(System.getProperty("user.dir"), "/config/fti.conf");
            ...
            Config testConfig = ConfigFactory.parseResources("test-fti.conf");
            logger.info("Config (" + (testConfig.entrySet().size() > 0 ? " yes " : " no  ") + "): test properties from resource 'test-fti.conf'");
            ...
            String file = System.getProperty("fti.conf.file");
            Config cmdLineConfigFile = file != null ? ConfigFactory.parseFile(new File(file)) : ConfigFactory.empty();
            logger.info("Config (" + (cmdLineConfigFile.entrySet().size() > 0 ? " yes " : " no  ") + "): user properties from -Dfti.conf.file file '" + file + "'");
            ...
            if (genesisJson.getConfig() != null && genesisJson.getConfig().isCustomConfig()) {
                ...
                if (config.hasPath("blockchain.config.name")) {
                    ...
                    try {
                        ...
                        throw new RuntimeException("The class specified via blockchain.config.class '" + className + "' is not instance of com.blockchain.fti.config.BlockchainForkConfig", e);
    ...
    public int defaultP2PVersion() {
        return config.hasPath("peer.p2p.version") ? config.getInt("peer.p2p.version") : 5;
    }

    @ValidateMe
    public int rlpxMaxFrameSize() {
        return config.hasPath("peer.p2p.framing.maxSize") ? config.getInt("peer.p2p.framing.maxSize") : Integer.MAX_VALUE >> 1;
    }
