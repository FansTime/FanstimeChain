...
import com.fanstime.fti.datasource.*;
...
import com.fanstime.fti.validator.*;
import com.fanstime.fti.vm.DataWord;
import com.fanstime.fti.vm.program.ProgramPrecompile;
...
public class CommonConfig {
    ...
    public void fastSyncCleanUp() {
        byte[] fastsyncStageBytes = blockchainDB().get(HashUtil.sha3("Key in state DB indicating fastsync stage in progress".getBytes()));
        ...
        FtiListener.SyncState syncStage = FtiListener.SyncState.values()[fastsyncStageBytes[0]];

        if (!systemProperties().isFastSyncEnabled() || syncStage == FtiListener.SyncState.UNSECURE) {
    ...
    @Bean(name = "FtiListener")
    public CompositeFtiListener ftiListener() {
        return new CompositeFtiListener();
    }
    ...
    public BlockHeaderValidator headerValidator() {
        ...
        List<BlockHeaderRule> rules = new ArrayList<>(asList(
                ...
                EthashRule.createRegular(systemProperties(), ftiListener()),
