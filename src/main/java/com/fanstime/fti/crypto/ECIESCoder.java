...
import com.fanstime.fti.ConcatKDFBytesGenerator;
...
import static com.fanstime.fti.crypto.ECKey.CURVE;
...
public class ECIESCoder {
    ...
    public static byte[] decrypt(ECPoint ephem, BigInteger prv, byte[] IV, byte[] cipher, byte[] macData) throws InvalidCipherTextException {
        ...
        FtiIESEngine iesEngine = new FtiIESEngine(
    ...
    public static byte[] encrypt(ECPoint toPub, byte[] plaintext, byte[] macData) {
        ...
        FtiIESEngine iesEngine = makeIESEngine(true, toPub, prv, IV);
...
//        };
    ...
    private static FtiIESEngine makeIESEngine(boolean isEncrypt, ECPoint pub, BigInteger prv, byte[] IV) {
        AESEngine aesFastEngine = new AESEngine();
