 ...
 * use {@link #toEthNotation()} to convert Jacobian coords to Fti encoding <br/>
 *
 */
public abstract class BN128<T extends Field<T>> {
    ...
    /**
     * Point at infinity in Fti notation: should return (0; 0; 0),
    ...
    public BN128<T> toEthNotation() {
        ...
        // affine zero is (0; 1; 0), convert to Fti zero: (0; 0; 0)
        if (affine.isZero()) {
            return zero();
        } else {
            return affine;
