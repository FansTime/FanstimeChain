...
import com.fanstime.fti.datasource.NodeKeyCompositor;
import com.fanstime.fti.datasource.rocksdb.RocksDbDataSource;
...
import com.fanstime.fti.db.StateSource;
...
import com.fanstime.fti.trie.TrieKey;
import com.fanstime.fti.util.*;
...
import static com.fanstime.fti.listener.FtiListener.SyncState.*;
import static com.fanstime.fti.trie.TrieKey.fromPacked;
import static com.fanstime.fti.util.ByteUtil.toHexString;
import static com.fanstime.fti.util.CompactEncoder.hasTerminator;

/**
 * Created by Jay Nicolas on 24.04.2018.
 */
@Component
public class FastSyncManager {
    ...
    public SyncStatus getSyncState() {
        ...
        FtiListener.SyncState syncStage = getSyncStage();
        switch (syncStage) {
            case UNSECURE:
...
//                idle.getFtiHandler().setStatus(SyncState.IDLE);
                return false;
            }
        } else {
            return false;
    ...
    private void setSyncStage(FtiListener.SyncState stage) {
        if (stage == null) {
    ...
    private FtiListener.SyncState getSyncStage() {
        byte[] bytes = blockchainDB.get(FASTSYNC_DB_KEY_SYNC_STAGE);
        if (bytes == null) return UNSECURE;
        return FtiListener.SyncState.values()[bytes[0]];
    }
...
//        }
    ...
    private void syncSecure() {
        ...
        setSyncStage(FtiListener.SyncState.SECURE);
    ...
    private void syncBlocksReceipts() {
        ...
        if (!config.fastSyncSkipHistory()) {
            ...
            setSyncStage(FtiListener.SyncState.COMPLETE);
    ...
    public void main() {
        ...
        if (blockchain.getBestBlock().getNumber() == 0 || getSyncStage() == SECURE || getSyncStage() == COMPLETE) {
            ...
            pool.setNodesSelector(handler -> false);

            try {
                FtiListener.SyncState origSyncStage = getSyncStage();
                ...
                switch (origSyncStage) {
                    case UNSECURE:
                        ...
                        if (pivot.getNumber() == 0) {
                            ...
                            syncManager.initRegularSync(FtiListener.SyncState.COMPLETE);
                            return;
                        }
                    ...
                    case SECURE:
                        if (origSyncStage == SECURE) {
                            ...
                            syncManager.initRegularSync(FtiListener.SyncState.UNSECURE);
                        }
                    ...
                    case COMPLETE:
                        if (origSyncStage == COMPLETE) {
                            ...
                            logger.info("Initializing regular sync");
                            syncManager.initRegularSync(FtiListener.SyncState.SECURE);
                        }
    ...
    private void fireSyncDone(FtiListener.SyncState state) {
            ...
            while (true) {
                ...
                if (allIdle.size() >= MIN_PEERS_FOR_PIVOT_SELECTION || forceSyncRemains < 0 && !allIdle.isEmpty()) {
                    ...
                    for (Channel channel : allIdle) {
                        if (bestPeer.getFtiHandler().getBestKnownBlock().getNumber() < channel.getFtiHandler().getBestKnownBlock().getNumber()) {
                            bestPeer = channel;
                        }
                    }
                    bestKnownBlock = bestPeer.getFtiHandler().getBestKnownBlock();
        ...
        if (bestIdle != null) {
            try {
                ListenableFuture<List<BlockHeader>> future =
                        bestIdle.getFtiHandler().sendGetBlockHeaders(pivotBlockHash, 1, 0, false);
    ...
    private Pair<BlockHeader, Long> getPivotHeaderByNumber(long pivotBlockNumber) throws Exception {
        ...
        if (!allIdle.isEmpty()) {
            try {
                ...
                for (Channel channel : allIdle) {
                    ListenableFuture<List<BlockHeader>> future =
                            channel.getFtiHandler().sendGetBlockHeaders(pivotBlockNumber, 1, false);
                    result.add(future);
                }
